...
namespace OnPoolServer
{
    public class OnPoolServer
    {
                ...
                case QueryType.Server:
                    switch (query.Method)
                    {
                        ...
                        case "OnPoolUpdated":
                        {
                            ...
                            pool.OnClientChange(client, () => { respond(GetClientByPool(query.Get("PoolName"))); });
        ...
        public bool FowardMessage(SocketManager socketManager, Query message, Action<Query> callback)
        {
            ...
            messageResponses[socketManager.Id + responseKey] = (q) => { callback(q); };
        ...
        {
            messageResponses[socketManager.Id + message.RequestKey] = (q) => { callback(q); };
